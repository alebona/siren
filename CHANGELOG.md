...
- `siren.profile`/`siren.profile_block` and `siren-perf` — pro-tier performance metrics. Samples are buffered locally and uploaded in the background (batched, never a blocking call per invocation), so `siren-perf top` can rank real bottlenecks (by total time spent) across runs over time instead of one call on one machine. `sample_rate`/`min_duration_ms` control volume on hot paths; `siren.perf_flush()` forces an upload before a short script exits.
